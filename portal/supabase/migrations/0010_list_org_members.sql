-- 0010_list_org_members.sql
--
-- A member-management UI needs to show WHO is on a team -- but
-- organization_members only ever stores user_id (a bare uuid), never an
-- email or display name (email lives in auth.users, which `authenticated`
-- has zero grants on at all -- see 0001_schema.sql's header on why only
-- auth.users.id is ever depended on elsewhere in this project). Without
-- this function, the Next.js layer has no sanctioned way to resolve "whose
-- membership row is this" into something a human can read.
--
-- SECURITY DEFINER, same shape as every other function in
-- 0007_org_management_functions.sql: checks the caller is actually a
-- member of p_org_id first, then joins organization_members to auth.users
-- *inside* the function body. This is not a convenience wrapper around
-- something the client could do itself -- `authenticated` has no grants on
-- auth.users whatsoever, so a raw client-side join is impossible; this
-- function is the only path, and it deliberately returns only email (never
-- any other auth.users column), scoped to exactly the members of one org
