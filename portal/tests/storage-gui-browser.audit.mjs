...
(async () => {
  ...
  // Exercise the newly added wizard through real server actions.
  ...
  const wizard = page.locator('div.surface').filter({ has: page.getByRole('heading', { name: 'Connect storage', exact: true }) });
  ...
  assert.match(await wizard.locator('code').innerText(), /^[0-9a-f]{40}$/);
  checks.push('New AWS wizard creates a unique external ID through authenticated server action');
  await wizard.getByRole('button', { name: 'Cancel', exact: true }).click();
  await page.reload();
  const lifecycle = { disconnect_buttons: await page.getByRole('button', { name: /disconnect/i }).count(), rotate_buttons: await page.getByRole('button', { name: /rotate|replace credentials|reconnect/i }).count(), pending_setup_visible: await page.getByText('GUI audit AWS draft', { exact: true }).count() };
  fs.writeFileSync(path.join(screenshots, 'gui-lifecycle.json'), JSON.stringify(lifecycle, null, 2));
  await page.getByRole('button', { name: 'Connect storage', exact: true }).click();
  await page.getByRole('button', { name: 'MinIO / S3-compatible', exact: true }).click();
  await wizard.getByLabel('Destination name', { exact: true }).fill('GUI audit local rejection');
  await wizard.getByLabel('Endpoint URL', { exact: true }).fill('http://127.0.0.1:19000');
  await wizard.getByLabel('Bucket', { exact: true }).fill(fixture.bucket);
  await wizard.getByLabel('Access key ID', { exact: true }).fill('nonsecret-fixture');
  await wizard.getByLabel('Secret access key', { exact: true }).fill('nonsecret-fixture');
  await wizard.getByRole('button', { name: 'Test and connect', exact: true }).click();
  await wizard.getByRole('alert').filter({ hasText: 'private or reserved' }).waitFor();
  checks.push('MinIO wizard rejects local/private endpoint through authenticated server action');
  await wizard.getByRole('button', { name: 'Cancel', exact: true }).click();
  const scans = [];
